...
import { loginThunk, restoreTokenThunk } from '../store/authThunks';
...
export default function LoginForm() {
  ...
  const { isLoading, token, authenticatorType, isRestoring } = useAppSelector(s => s.auth);
  ...
  useEffect(() => {
    if (!isRestoring) {
      router.push(isAuthorized ? '/management' : '/auth/login');
    }
  }, [isRestoring]);
  ...
  useEffect(() => {
    ...
    if (authenticatorType !== 'None' && authenticatorType !== null) {
      alert('authenticator gerekli');
    }
  }, [token]);

  const { control, handleSubmit, reset } = useForm<LoginRequest>({
    defaultValues: { authenticatorCode: '', email: '', password: '' },
  });
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Input
                        type="email"
                        placeholder="Email"
                        {...field} // value ve onChange field'dan geliyor
                      />
                    )}
                  />
                </div>
